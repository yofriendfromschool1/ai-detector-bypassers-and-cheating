...
async function syncPaymentStatus() {
  try {
    ...
    if (!response.ok) {
      console.warn(`[syncPaymentStatus] server ${response.status} → marking paid:false`);
      chrome.storage.local.remove(
        ['email', 'paid', 'lifetimePaid', 'customerEmail'],
        () => chrome.runtime.lastError && console.warn(chrome.runtime.lastError)
      );
    ...
    const isPaid = !!data.loggedIn;
    const newEmail = data.updatedEmail || email;

    chrome.storage.local.set(
      { email: newEmail, paid: isPaid, lifetimePaid: lifetimePaid || false },
      () => chrome.runtime.lastError && console.warn(chrome.runtime.lastError)
    );
